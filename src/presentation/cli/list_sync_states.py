...
def list_sync_states(db_path: str, format: str):
    ...
    try:
        ...
        echo_output(format, sync_states)
...
def echo_output(format, sync_states):
    if format == 'json':
        # JSON output
        echo_json(sync_states)
    else:
        # Table output
        if not sync_states:
            click.echo("No repositories configured for synchronization.")
        echo_tabulate(sync_states)


def echo_tabulate(sync_states):
    table_data = [
        [
            state.repository_id,
            state.provider_type,
